...
class Generate:
	...
	def __init__(self, dir, apiName):
		...
		self.f_head = open(os.path.join(os.path.abspath('..\..\hf_ctp_c_proxy\ctp_{0}'.format(self.ClassName)), '{0}.h'.format(apiName)), 'w', encoding='utf8')

		self.f_cpp = open(os.path.join(os.path.abspath('..\..\hf_ctp_c_proxy\ctp_{0}'.format(self.ClassName)), '{0}.cpp'.format(apiName)), 'w', encoding='utf8')
